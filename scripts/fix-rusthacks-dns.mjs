/**
 * Attach arcraidershacks.net to Worker islecheat and create DNS CNAMEs.
...
const DOMAINS = ['arcraidershacks.net', 'www.arcraidershacks.net'];
...
async function ensureCname(name, content) {
	const fqdn = name.includes('.') ? name : `${name}.arcraidershacks.net`;
	const existing = await listDns();
	const hit = existing.find(
		(r) => r.name === fqdn || r.name === name || (name === '@' && r.name === 'arcraidershacks.net'),
	);
	if (hit) {
	...
	const recordName = name === 'arcraidershacks.net' || name === '@' ? 'arcraidershacks.net' : name;
...
async function main() {
	...
	await ensureCname('arcraidershacks.net', TARGET);
